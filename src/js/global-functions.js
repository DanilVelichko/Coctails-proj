...
export const renderButtonInternals = idDrink => {
  return `<div class="box__addTo__remove"> ${
    favorites.includes(idDrink) ? `Remove` : `Add to`
  }
...
async function asyncRender() {
  ...
  for (let i = 0; i < itemsPerPage; i++) {
    if (elBtn) {
      await loadDataLetter().then(drink => {
        point.galleryUl.insertAdjacentHTML('beforeend', renderCard(drink));
